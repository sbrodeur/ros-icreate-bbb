...
namespace create {
  ...
  bool Create::isWheeldrop() const {
    if (data->isValidPacketID(ID_BUMP_WHEELDROP)) {
      return (GET_DATA(ID_BUMP_WHEELDROP) & 0x1C) != 0;
    }
    else {
  ...
  bool Create::isCasterWheeldrop() const {
    if (data->isValidPacketID(ID_BUMP_WHEELDROP)) {
      return (GET_DATA(ID_BUMP_WHEELDROP) & 0x10) != 0;
    }
    else {
      CERR("[create::Create] ", "Wheeldrop sensor not supported!");
      return false;
  ...
  bool Create::isRightCliff() const{
  ...
  bool Create::isFrontLeftCliff() const{
  ...
  bool Create::isFrontRightCliff() const{
  ...
  uint16_t Create::getWallSignal() const {
	if (data->isValidPacketID(ID_WALL_SIGNAL)) {
	  return GET_DATA(ID_WALL_SIGNAL);
	}
	else {
	  CERR("[create::Create] ", "Wall sensor not supported!");
	  return 0;
  ...
  uint16_t Create::getCliffLeftSignal() const {
	if (data->isValidPacketID(ID_CLIFF_LEFT_SIGNAL)) {
	  return GET_DATA(ID_CLIFF_LEFT_SIGNAL);
	}
	else {
	  CERR("[create::Create] ", "Cliff sensor not supported!");
	  return 0;
	}
  }

  uint16_t Create::getCliffFrontLeftSignal() const {
  ...
  uint16_t Create::getCliffFrontRightSignal() const {
  ...
  uint16_t Create::getCliffRightSignal() const {
  ...
  int16_t Create::getRequestedLeftWheelVel() const {
	if (data->isValidPacketID(ID_LEFT_VEL)) {
	  // This is a standards compliant way of doing unsigned to signed conversion
	  uint16_t velocityRaw = GET_DATA(ID_LEFT_VEL);
	  int16_t velocity;
	  std::memcpy(&velocity, &velocityRaw, sizeof(velocity));
	  return velocity;
	}
	else {
	  CERR("[create::Create] ", "Requested velocity sensor not supported!");
	  return 0;
	}
  }

  int16_t Create::getRequestedRightWheelVel() const {
	if (data->isValidPacketID(ID_RIGHT_VEL)) {
	  // This is a standards compliant way of doing unsigned to signed conversion
	  uint16_t velocityRaw = GET_DATA(ID_RIGHT_VEL);
	  int16_t velocity;
	  std::memcpy(&velocity, &velocityRaw, sizeof(velocity));
	  return velocity;
	}
