...
class Create:
	...
	def __init__(self, tty="/dev/ttyUSB0"):
		...
		self.packets = (
			...
			(14,1,mask,("na","na","na","wheelLeftOvercurrent","wheelRightOvercurrent","na","na","na")),
			...
			(41,2,signed,("requestedRightVelocity",)),
			(42,2,signed,("requestedLeftVelocity",)),
			)
	...
	def sleep(self):
		"""Set the iCreate to passive mode."""
		
		#self.__stopMonitor()
		
		self.__sendNow(128,128,128)
		sleep(1)
	...
	def resume(self):
		"""Set the iCreate back to active mode."""
	...
	def __startMonitor(self):
		self.monitor = Thread(target=self.__monitor)
		self.monitorEnabled = True
		self.monitor.start()

	def __stopMonitor(self):
		self.monitorEnabled = False
	...
	def __monitor(self):
		while(self.monitorEnabled):

			then = datetime.now()
			self.__sendAll() #send queued commands

			self.send(149,len(self.packets),*[i[0] for i in self.packets]) #read sensor packets
			self.__sendAll() 

			bytes = self.__read(sum([i[1] for i in self.packets]))
				...
				for packet in self.packets:
					...
					for result in results:
						...
						if (packet[3][index] == "distance"):
							...
							data[packet[3][index]] = self.__addDistance(result)
						if (packet[3][index] == "angle"):
							data['d_angle'] = result
							data[packet[3][index]] = self.__addAngle(result)
						index = index + 1
			...
			if (elapsed < self.PERIOD*1000.):
				sleep((self.PERIOD*1000. - elapsed)/1000.)
